...
const fs = require("fs");
const variables = {
    "tweetId": "1299986517935493121",
    "count": 20,
    "cursor": "HBbGksas5LGQxC4AAA==",
...
res = axios.get(encodeURI(baseURL + formData), {
    headers: {
    ...
    .then(response => {
        ...
        xTweetRetweetListCallback(responseData);
    })
...
function xTweetRetweetListCallback(responseData) {
    ...
    let retweeters = [];
    for (let i = 0; i < responseData.data.retweeters_timeline.timeline.instructions[0].entries.length; i++) {
        try {
